...
{
    [TestClass]
    [TestCategory(nameof(AkatsukiAssetBundleEncryptionAlgorithm))]
    public sealed class AkatsukiAssetBundleEncryptionAlgorithmTests
    {
        [TestMethod]
        [DataRow("00077028d3cd0fc37fdea2376aec4e41_6959df3ffb10f42513e85d20de868b67")]
        public void CompareImplementations(string bundleName)
        {
            string bundlePath = Path.Combine("Resources/AkatsukiAssetBundles", bundleName);
            string bundleInfoPath = $"{bundlePath}.json";
            byte[] bytes = File.ReadAllBytes(bundlePath);
            BundleInfo bundleInfo = JsonSerializer.Deserialize<BundleInfo>(File.ReadAllText(bundleInfoPath));

            // Only proceed with decryption comparison if the bundle is encrypted
            if (bundleInfo.Compression == 3)
            {
                // Get result from new implementation
                var newImpl = new AkatsukiAssetBundleEncryptionAlgorithm(
                    bundleInfo.BundleName,
                    bundleInfo.FileSize,
                    bundleInfo.Hash,
                    bundleInfo.Crc
                );
                byte[] newResult = newImpl.Decrypt(bytes);

                // Get result from reference implementation
                var bundle = new Bundle(
                    RelativePath: "",
                    BundleName: bundleInfo.BundleName,
                    Hash: bundleInfo.Hash,
                    Crc: bundleInfo.Crc,
                    FileSize: (int)bundleInfo.FileSize,
                    FileMd5: "", // Not needed for decryption
                    Compression: bundleInfo.Compression,
                    UserData: "" // Not needed for decryption
                );

                byte[] referenceResult;
                using (var ms = BundleCrypto.DecryptBundle(bytes, bundle))
                {
                    referenceResult = new byte[ms.Length];
                    ms.Read(referenceResult, 0, (int)ms.Length);
                }

                Assert.AreEqual(referenceResult.Length, newResult.Length, "Decrypted data lengths differ");

                for (int i = 0; i < referenceResult.Length; i++)
                {
                    Assert.AreEqual(
                        referenceResult[i],
                        newResult[i],
                        $"Mismatch at byte {i}: Expected {referenceResult[i]:X2}, Got {newResult[i]:X2}"
                    );
                }

                File.WriteAllBytes("Decrypted Akatsuki Bundle.bytes", newResult);
            }
        }


        internal record Bundle(string RelativePath, string BundleName, string Hash, long Crc, int FileSize, string FileMd5, int Compression, string UserData);
